    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if peca_atual is not None and prev is None:
                ...
                if peca_prev is not None and (peca_atual is None or peca_atual.cor != peca_prev.cor):
                    if turn == peca_prev.cor:
                        print('movimentacao de ', peca_prev.tipo, peca_prev.cor, ' para ', highlighted_square)
                        sucesso = table.move(peca_prev, highlighted_square)
                        if sucesso:
                            turn = 'branco' if table.tabuleiro[linha][coluna].cor == 'preto' else 'preto'
                            if table.verify_check(turn):
                                if table.not_have_moves(turn):
                                    print('o jogador ', table.tabuleiro[linha][coluna].cor, 'ganhou')
                                else:
                ...
                elif peca_prev.cor == peca_atual.cor and peca_prev.tipo == 'RE' and peca_atual.tipo == 'T':
                    if turn == peca_prev.cor:
                        print('roque')
                        if coluna == 7:
